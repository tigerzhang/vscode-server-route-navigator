...
const DEBUG = !!(process.env.SRN_DEBUG === '1' || (process.env.DEBUG && process.env.DEBUG.includes('server-route-navigator')));
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('serverRouteNavigator.goToServerHandler', async (rangeOrPath) => {
        ...
        if (matches.length === 1) {
            ...
            const choice = await vscode.window.showInformationMessage(`Found 1 match: ${m.uri.fsPath}:${m.line + 1}\n${m.snippet}`, { modal: false }, openAction, previewAction);
...
class ServerRouteCodeLensProvider {
    constructor() {
...
function extractPathLiteralAt(document, position) {
...
function extractPathLiteralFromLine(document, lineNumber) {
    const line = document.lineAt(lineNumber).text;
    const regex = /(\"|\')(\/api\/[^\"']+)(\"|\')/g;
    const match = regex.exec(line);
    if (match)
        debugLog('extractPathLiteralFromLine', document.uri.fsPath, 'line', lineNumber, 'found', match[2]);
    return match ? match[2] : undefined;
}
async function findServerHandler(pathToSearch) {
            ...
            for (let i = 0; i < lines.length; i++) {
                if (lines[i].includes(normalizedPath)) {
                    ...
                    while (true) {
                        ...
                        searchPos = found + 1;
                        if (searchPos > 300)
                            break; // avoid scanning arbitrarily long lines
                    }
...
function extractActixRoutes(text) {
    const result = [];
    const lines = text.split(/\r?\n/);
...
function actixPatternMatches(routePattern, path) {
...
function isPotentialPathMatch(line, idx, segment) {
...
function isWithinAbsoluteUrl(line, idx) {
...
function isLastSegmentPlaceholder(route) {
    if (!route)
        return false;
